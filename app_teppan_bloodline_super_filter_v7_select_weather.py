...
with st.expander("競馬場・芝ダごとに指定する", expanded=True):
    st.caption("最大3会場まで同時指定できます。未使用の行は空欄のままでOKです。")
    ...
    for i in range(1, 4):
        ...
        with c2:
            surfaces = st.multiselect(
                f"芝ダ{i}",
                surface_options,
                default=[],
                key=f"manual_surface_{i}",
                help="芝だけ、ダだけ、または芝・ダ両方を選べます。",
            )
        ...
        with c4:
            going = st.selectbox(
                ...
                going_options,
                index=0,
                key=f"manual_going_{i}",
            )

        if place and surfaces:
