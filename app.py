...
with st.expander("TARGETから抜く推奨列を見る", expanded=True):
    st.code(
        "日付,場所,R,レース名,芝・ダ,距離,馬番,馬名,種牡馬,父タイプ名,母父名,母父タイプ名,性齢,所属,調教師,斤量,頭数,前芝・ダ,前距離,前走馬場状態,前走斤量,前走着順,前走着差,替,騎手,前騎手,休み明け～戦目",
        language="csv",
    )
...
st.subheader("天気・馬場状態の手入力補完")
st.caption("TARGETから天気・馬場状態が取れない場合だけ使います。空欄なら、その条件はスキップします。")

c1, c2 = st.columns(2)
with c1:
    manual_weather = st.selectbox("全体の天気", ["", "晴", "曇", "雨", "小雨", "雪"], index=0)
with c2:
    manual_going = st.selectbox("全体の馬場状態", ["", "良", "稍重", "重", "不良"], index=0)

with st.expander("競馬場・芝ダごとに指定する場合", expanded=False):
    st.caption("全体指定よりこちらを優先します。最大3会場×芝ダで同時指定できます。")
    manual_condition_csv = st.text_area(
        "補完CSV",
        value="競馬場,芝ダ,天気,馬場状態\n",
        height=140,
        placeholder="競馬場,芝ダ,天気,馬場状態\n東京,芝,晴,良\n東京,ダ,晴,稍重\n京都,芝,曇,良\n京都,ダ,曇,良\n新潟,芝,雨,稍重\n新潟,ダ,雨,重",
    )
    ...
    try:
        if input_mode == "CSV貼り付け":
            ...
            if uploaded_file is None:
                ...
                st.stop()
            input_df = read_csv_smart(uploaded_file)
