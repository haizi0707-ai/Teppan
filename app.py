...
# TARGETで取得できないことがある項目。
# 空欄・列なしの場合は不一致にせず、条件判定をスキップ扱いにします。
OPTIONAL_NEUTRAL_ITEMS = {
    "天気",
    "馬場状態",
    "前走調教師",
    "前走所属",
...
def match_value(row, item, expected):
    ...
    # TARGETで取得できないことがある項目は、列なしならスキップ扱い
...
with st.expander("TARGETから抜く推奨列を見る", expanded=True):
    ...
    st.caption("父タイプ名が空欄でも種牡馬から補完します。前走着順・前走着差から、鉄板⭐️/強鉄板⭐️/超鉄板⭐️を自動判定します。天気・馬場状態・前走調教師が無い場合は、その条件だけスキップします。天気・馬場状態は画面から手入力補完もできます。結果は詳細版と、複合アプリ用の8列版の両方を出力できます。")
