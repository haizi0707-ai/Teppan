...
with st.expander("TARGETから抜く推奨列を見る", expanded=True):
    st.code(
        "日付,場,R,レース名,距離,馬番,馬名,種牡馬,父タイプ名,母父名,母父タイプ名,性齢,所属,調教師,斤量,頭数,前芝・ダ,前距離,前走馬場状態,前走斤量,前走着順,前走着差,替,騎手,前騎手,休み明け～戦目",
        language="csv",
    )
...
with st.expander("競馬場・芝ダごとに指定する", expanded=True):
    st.caption("最大3会場×芝/ダの6枠まで同時指定できます。芝とダで馬場状態が違う場合も分けて指定できます。TARGETの列が「場」「距離=芝2890/ダ1800」でも自動変換します。")
