...
# GitHub/iPhoneアップロード時にファイル名が微妙に変わっても拾うための候補検索
def find_csv_file(exact_name, keywords):
    base = Path(".")
    exact = base / exact_name
    if exact.exists():
        return exact
    candidates = []
    for f in base.glob("*.csv"):
        name = f.name.replace(" ", "").replace("　", "")
        if all(k.replace(" ", "").replace("　", "") in name for k in keywords):
            candidates.append(f)
    if candidates:
...
# ファイル存在チェック
logic_path = find_csv_file(LOGIC_FILE, ["TOP5並列", "修正版", "最終ロジック辞書"])
course_path = find_csv_file(COURSE_FILE, ["コース別", "採用保留除外判定"])
...
if not logic_path.exists() or not course_path.exists():
    ...
    found = [f.name for f in Path(".").glob("*.csv")]
    if found:
        st.code("\n".join(found))
    else:
        st.code("CSVファイルが見つかりません")
    st.stop()

st.sidebar.caption(f"ロジック辞書: {logic_path.name}")
st.sidebar.caption(f"コース判定: {course_path.name}")
