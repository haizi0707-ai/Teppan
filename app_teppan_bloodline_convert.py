...
st.info("予想CSVはGitHubに置かず、この下の入力欄に貼り付けてください。父タイプ名が空欄でも、種牡馬があれば一部自動補完します。")

with st.expander("TARGETから抜く推奨列を見る", expanded=True):
    st.code(
        "日付,場所,R,レース名,芝・ダ,距離,馬番,馬名,種牡馬,父タイプ名,母父名,母父タイプ名,性齢,所属,調教師,斤量,頭数,天気,馬場状態,前芝・ダ,前距離,前走馬場状態,前走斤量,替,騎手,前騎手,休み明け～戦目",
        language="csv",
    )
    st.caption("父タイプ名が空欄でも種牡馬から補完します。母父タイプ名が空欄でも母父名から補完します。性齢・Ｒ・休み明け～戦目などの表記ゆれも吸収します。")
