...
def diagnose_bloodline_conversion_files():
    """
    血統タイプ変換表.csv が本当に見えているか、列名を認識できているかを画面表示するための診断。
    """
    rows = []
    for f in csv_files():
        name = f.name
        is_candidate = any(key in name for key in ["血統タイプ変換", "種牡馬タイプ変換", "bloodline_type"])
        if not is_candidate:
            continue
...
def build_bloodline_type_map(logic_df):
    ...
    base = {
        ...
        # ユーザー追加：血統タイプ変換表が読めない環境でも補完するための固定追加
        "BeauLiam": "ネイティヴダンサー系",
        "Boltd'Oro": "ニアークティック系",
        "Kameko": "ニアークティック系",
        "LopedeVega": "ニアークティック系",
        "Maurice": "ロイヤルチャージャー系",
        "PalacePier": "ニアークティック系",
        "Peltzer": "ニアークティック系",
        "Sottsass": "ニアークティック系",
        "Tom'sd'Etat": "ネイティヴダンサー系",
        "VictorLudorum": "ニアークティック系",
        "WillTakeCharge": "ネイティヴダンサー系",
        "アスクピーターパン": "ニアークティック系",
        "インカンテーション": "ナスルーラ系",
        "サングレーザー": "ロイヤルチャージャー系",
        "シルポート": "ネイティヴダンサー系",
        "セイウンコウセイ": "ネイティヴダンサー系",
        "ブルドッグボス": "ロイヤルチャージャー系",
        "CairoPrince": "ネイティヴダンサー系",
        "CircularQuay": "ネイティヴダンサー系",
        "CityZip": "ネイティヴダンサー系",
        "DeepImpact": "ロイヤルチャージャー系",
        "Firebreak": "ネイティヴダンサー系",
        "Golan": "ニアークティック系",
        "Magician": "ニアークティック系",
        "Murtajill": "ニアークティック系",
        "Myboycharlie": "ニアークティック系",
        "Oscar": "ニアークティック系",
        "RecklessAbandon": "ニアークティック系",
        "SeaTheMoon": "ニアークティック系",
        "Skipshot": "ネイティヴダンサー系",
        "SoldierHollow": "ニアークティック系",
        "SuperEasy": "ニアークティック系",
        "SuperSaver": "ネイティヴダンサー系",
        "Tizway": "ネイティヴダンサー系",
        "カフェラピード": "ロイヤルチャージャー系",
...
with st.expander("血統タイプ変換表の読み込み状況", expanded=True):
    ...
    st.info("この版では、今回送っていただいた未変換馬は app.py 内にも固定追加済みです。変換表が読めなくても BeauLiam / Boltd'Oro / Kameko などは補完されます。")
...
ヘッダーなしTARGET出力もそのまま読み込めます。""",
    ...
    st.caption("父タイプ名が空欄でも種牡馬から補完します。前走着順・前走着差から、鉄板⭐️/強鉄板⭐️/超鉄板⭐️を自動判定します。性齢・Ｒ・休み明け～戦目などの表記ゆれも吸収します。結果は詳細版と、複合アプリ用の9列版（日付,競馬場,R,レース名,馬番,馬名,鉄板ランク,判定,印）を出力できます。複合アプリ用CSVは7〜12Rのみです。")
...
with st.expander("競馬場・芝ダごとに指定する", expanded=True):
    st.caption("最大3会場まで同時指定できます。天気は会場ごとに1つ、馬場状態は芝・ダで別々に指定できます。TARGETのヘッダーなしCSV、列が「場」、Rが「新1」、距離が「芝2890/ダ1800」でも自動変換します。")
...
if run:
    try:
        ...
        if result.empty:
            ...
            # 複合アプリ用CSV：7〜12Rのみ、同一レース内で鉄板ランク順に印を付与
            ...
            simple_df["R_num"] = simple_df["R"].map(_r_num)

            # 7〜12Rのみ出力
            simple_df = simple_df[simple_df["R_num"].between(7, 12, inclusive="both")].copy()

            # 同じ馬が父系/母父系などで重複した場合は、強い鉄板ランク側を残す
            simple_df["_rank_priority"] = simple_df["鉄板ランク"].map(_teppan_rank_priority)
            simple_df["_fukusho"] = simple_df.get("検証複勝率%", "").map(_to_float) if "検証複勝率%" in simple_df.columns else 0.0
            simple_df["_lift"] = simple_df.get("リフト", "").map(_to_float) if "リフト" in simple_df.columns else 0.0
            simple_df["_horse_no"] = simple_df["馬番"].map(_r_num).fillna(999)

            simple_df = (
                simple_df
                .sort_values(
                    ["日付", "競馬場", "R_num", "_rank_priority", "_fukusho", "_lift", "_horse_no"],
            ...
            simple_df = simple_df[composite_cols]
            simple_csv = simple_df.to_csv(index=False, encoding="utf-8-sig")

            st.download_button(
                "結果CSVをダウンロード（複合アプリ用・7〜12R）",
                simple_csv,
                file_name="teppan_bloodline_result_for_composite_7-12R.csv",
                mime="text/csv",
                use_container_width=True,
            )
